...
ghost_blue::ghost_blue(QWidget * parent,const int x_pos[],const int y_pos[],object_key color)
...
void ghost_blue::object_show(){
    over = false;
    ghost_label->show();
}

void ghost_blue::start_ctrl(){
    start = ! start;
    if(start == false){
        over = false;//for act
        weak = false;//for act
...
void ghost_blue::man_pos(int man__r,int man__c){
    man_r = man__r;
    man_c = man__c;
}


void ghost_blue::move_method(){
    //horse
    if(start && !over && !die){
        int square_distance;
...
void ghost_blue::update_to_game(){
    if(start && !over && !die)
        emit ghost_done(id,ghost_row,ghost_col);
}

int ghost_blue::distance(int gr,int gc,int mr,int mc){
    return (gr - mr) * (gr - mr) + (gc - mc) * (gc - mc);
}

int ghost_blue::tran_x(int xpos){
    int x = (xpos - map_info::play_region_x ) / block_width;
    x = xpos - map_info::play_region_x < 0 ? - 1 : x ;
    return x;
}

int ghost_blue::tran_y(int ypos){
